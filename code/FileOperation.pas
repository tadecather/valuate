...
uses  SysUtils, Variants, Classes,Windows, Messages, Dialogs, DB, ADODB,
    ...
    function getStringOf_Date(date : TDateTime): String;
    function jkFileToDataBase(jkType: String; filePath,fileName: String;ywrq: TDateTime): Boolean;
    function jkFileAlready(SQLString: String): Boolean;
...
// 判断接口表中的数据是否已经存在 传入参数为 SQL 语句
function jkFileAlready(SQLString: String): Boolean;
Var qry1: TADOQuery;
begin
  qry1 := TADOQuery.Create(MainForm);
  qry1.ConnectionString :=  MainForm.conMainForm.ConnectionString;
  with qry1 do
  begin
    Close;
    SQL.Clear;
